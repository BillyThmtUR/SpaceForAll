...
export default function SaturneModel() {
  const [loadingProgress, setLoadingProgress] = useState(0);

  return (
    <div style={{ width: '100%', height: '100%' }}>
      <Canvas style={{ height: '100%'}} camera={{ position: [0, 0, 10], fov: 50 }} >
...
function SceneContent({ setLoadingProgress }) {
  ...
  const { currentTexture, cloudTexture } = useMemo(() => {
    const textureLoader = new THREE.TextureLoader(manager);
    return {
      currentTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/8k_saturn.jpg`),
      cloudTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/saturnclouds.jpg`),
    };
  }, [manager]);

  const { nodes, materials } = useGLTF(`${process.env.PUBLIC_URL}/data/textures/saturnRing.glb`);
  ...
  return (
    <>
      ...
      <directionalLight color='#ffffff' position={[-5, 5, 5]} intensity={0.5} />
      ...
      <pointLight color='#ffffff' position={[-5, 5, 5]} intensity={10} />
