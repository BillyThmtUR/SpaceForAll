...
export default function VenusModel({ opacity }) {
  const [loadingProgress, setLoadingProgress] = useState(0);

  return (
    <div style={{ width: '100%', height: '100%' }}>
      <Canvas camera={{ position: [0, 0, 10], fov: 50 }}>
        <SceneContent opacity={opacity} setLoadingProgress={setLoadingProgress} />
...
function SceneContent({ opacity, setLoadingProgress }) {
  ...
  const { currentTexture, cloudTexture, bumpTexture } = useMemo(() => {
    const textureLoader = new THREE.TextureLoader(manager);
    return {
      currentTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/8k_venus.jpg`),
      cloudTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/venusAtmosphere.jpg`),
      bumpTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/venusBump.jpg`),
    };
  }, [manager]);
  ...
  return (
    <>
      <directionalLight color='#ffffff' position={[180, -8, -10]} intensity={1.5} />
    ...
    {/* Atmosphere */}
    <mesh position={[0, 0, 0]} ref={meshRef2}>
