...
function SceneContent({ setLoadingProgress }) {
  ...
  const { currentTexture, bumpTexture } = useMemo(() => {
    ...
    return {
      currentTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/8k_moon.jpg`),
      bumpTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/moonBump.jpg`),
    };
  }, [manager]);
