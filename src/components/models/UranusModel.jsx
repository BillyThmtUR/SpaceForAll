...
function SceneContent({ setLoadingProgress }) {
  ...
  const { currentTexture, bumpTexture } = useMemo(() => {
    ...
    return {
      currentTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/8k_uranus.jpg`),
      bumpTexture: textureLoader.load(`${process.env.PUBLIC_URL}/data/textures/uranusBump.jpg`),
    };
  }, [manager]);
  ...
  return (
    <>
      ...
      <directionalLight color='#ffffff' position={[-80, 180, 180]} intensity={2} />
    ...
    {/* Uranus */}
    <mesh position={[0, 0, 0]} ref={meshRef} rotation={[0, 0, THREE.MathUtils.degToRad(98)]}>
